...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      ...
      switch (true) {
        ...
        case "author" in args && "genre" in args:
          return booksByAuthor = books.filter(b => b.author === args.author).filter(b => b.genres.find(g => g === args.genre))
        default:
          return null
      }
    },
  ...
  Author: {
    bookCount: (root, args) => {
      ...
      return books.filter(b => b.author === author).length
    }
  },
  Mutation: {
    addBook: (root, args) => {
      ...
      author.length === 0 ? authors = authors.concat({ name: args.author, born: null }) : null
      return book
    },
    deleteBook: (root, args) => {
      books = books.filter(b => b !== args.title)
      return `${args.title} deleted!`
    },
    addAuthor: (root, args) => {
      ...
      if (!args.born) {
        const newAuthor = { ...args, born: null }
